...
class DeepSeekService {
  ...
  // Get information about a detected species (bird or insect)
  static Future<Map<String, dynamic>> getSpeciesInformation(String speciesName, String speciesType) async {
    try {
      // Construct a prompt for the DeepSeek API
      final prompt = "Provide detailed information about the $speciesName, which is a ${speciesType} species. Include: \n"
          "1. Brief description and classification\n"
          "2. Habitat and geographic distribution\n"
          "3. Behavior and ecological role\n"
          "4. Conservation status (if applicable)\n"
          "Keep the response informative but concise (200-250 words).";
      ...
      if (response.statusCode == 200) {
        final jsonResponse = json.decode(response.body);
        // Extract the species information from the response
        final information = jsonResponse['choices'][0]['message']['content'];
        return {
          'success': true,
        ...
        return {
          'success': false,
          'message': 'Failed to get species information: ${response.statusCode}',
          'details': response.body,
        };
      }
    } catch (error) {
      return {
        'success': false,
        'message': 'Error getting species information: $error',
      };
    }
  }
  ...
  static getMockSpeciesInformation(String speciesName, String detectionType) {}
}